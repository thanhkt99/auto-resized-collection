...
class API {
    ...
    static private let BASE_URL = "https://api.500px.com/v1/"
    ...
    static private func makeURL(endpoint: Endpoints) -> URL {
    ...
    func request<T: Decodable>(endpoint: Endpoints,
                               httpMethod: String = "GET",
    ...
    private func executeRequest<T: Decodable>(publisher: URLSession.DataTaskPublisher) -> AnyPublisher<T, APIError> {
        publisher.tryMap { [weak self] data, response in
            try self?.processRespone(data: data, response: response) ?? Data()
        }
    ...
    private func processRespone(data: Data,
